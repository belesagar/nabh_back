...
class NabhGroupController extends Controller
{
    ...
    public function updateNabhGroup(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $request_data = $request->all();
            if (!empty($this->payload)) {
                $check_data = $this->nabh_group->select('nabh_group_id')->where('nabh_name',
                    $request_data['nabh_name'])->get();
                ...
                if (count($check_data) == 0 && $request_data['nabh_group_id'] == $check_data[0]['nabh_group_id']) {
                    ...
                    $response = $this->nabh_group->where('nabh_group_id',
                        $request_data['nabh_group_id'])->update($update_data);
                    if ($response) {
                        $return = array("success" => true, "error_code" => 0, "info" => "Success");
                    } else {
                        $return = array(
