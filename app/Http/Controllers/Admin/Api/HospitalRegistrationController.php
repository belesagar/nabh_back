...
class HospitalRegistrationController extends Controller
{
    ...
    {
        $list = $this->hospital_registration->orderBy('created_at', 'desc')->get()->toArray();
    ...
    public function updateHospitalData(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!empty($this->payload)) {
                ...
                $response = $this->hospital_registration->where('hospital_id',
                    $request_data['hospital_id'])->update($update_data);
                if ($response) {
                    $return = array("success" => true, "error_code" => 0, "info" => "Success");
                } else {
                    $return = array(
