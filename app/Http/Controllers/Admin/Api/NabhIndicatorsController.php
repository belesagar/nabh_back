                ...
                if($response_id > 0)
                {
                    if(!empty($request_data['validations']))
                    {
                        ...
                        foreach ($request_data['validations'] as $validation_value) {
                            ...
                            {
                                $validations[] = ["required" => true,"message" => "Field is required.","type" => "required"];
                            }
                            if($validation_value == "minlength")
                            {
                                $validations[] = ["minLength" => 3,"message" => "Field must be at least 3 characters long.","type" => "minLength"];
    ...
    public function getformInfo(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $data_response = $this->indicators_forms_fields->where('form_id', $request_data['form_id'])
                                ->with(['getValidations' => function($query) {
    ...
    public function updateFormData(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if(!empty($this->payload))
            {
                ...
                $response = $this->indicators_forms_fields->where('form_id', $request_data['form_id'])->update($update_data);
                if($response)
                {
                    if(!empty($request_data['validations']))
                    {
                        ...
                        if(!empty($validations))
                        {
                            ...
                            $validation_response = $this->indicators_forms_fields_validation->where('form_id', $request_data['form_id'])->update($validation_data);  
                            if(!$validation_response)
                            {
                                $return = array("success" => false,"error_code"=>1,"info" => "Something is wrong, please try again.");
                            } 
