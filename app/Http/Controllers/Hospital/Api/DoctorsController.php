...
class DoctorsController extends Controller
{
    ...
    public function List(Request $request) {
    	$list = $this->hospital_doctors->where("hospital_id",$this->payload['hospital_id'])->orderBy('created_at', 'desc')->get()->toArray();
    ...
    public function getInfo(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            $data_info = $this->hospital_doctors->where('doctor_id', $request_data['doctor_id'])->where("hospital_id",$this->payload['hospital_id'])
                                ->get();
    ...
    public function Edit(Request $request) {
        ...
        if ($validator->fails()) {
	        ...
	        if(!empty($this->payload))
	        {
	        	...
	        	$response = $this->hospital_doctors->where('doctor_id', $request_data['doctor_id'])->where("hospital_id",$this->payload['hospital_id'])->update($update_data);
	        	if($response)
	        	{
    ...
    public function typeList(Request $request) {
    	$list = $this->hospital_doctors_type->where("hospital_id",$this->payload['hospital_id'])->orderBy('created_at', 'desc')->get()->toArray();
        $data = array("list" => $list);
        $return = array("success" => true,"error_code"=>0,"info" => "Success","data" => $data);
        return json_encode($return);
    }
